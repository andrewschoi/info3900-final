...
```{r}
# The following 2 lines causes a compliation issue when knitting, uncomment if you run yourself
...
relevant_df = combined_df %>% 
  ...
  filter(H4ED1 <= 4) %>% # Filter out don't know for H4ED1
  mutate(H4WP4 = case_when(H4WP4 >= 97 ~ NA, .default = H4WP4),
         ...
         H4ED1 = case_when(H4ED1 %in% c(1, 2) ~ 1, # Graduated high school
                          H4ED1 %in% c(3,4) ~ 0),  # Did not graduate high school, This is dichotomizing the outcome variable 
...
```{r}
library(MatchIt)
# treatment formula based off of what the sufficient adjustment set is (those are the variables we include): 

# NOT THIS: treatment_formula<-Mother.Incarcerated~Household.Income+Sex+Household.Assets+Family.Support.Education+Family.Responsibilities+Neighborhood.Safety

treatment_formula2<-Mother.Incarcerated~Household.Income+Neighborhood.Safety

#Perform Matching
clean_relevant_df<-relevant_df %>% select(-Mother.Incarcerated.Times, -Mother.Incarcerated.Respondent.Age) %>% drop_na()

matching<-matchit(treatment_formula2,
                  method="nearest", 
                  distance="glm",
                  estimand="ATT", 
                  data=clean_relevant_df)
matching
summary(matching)
summary(matching)$nn

# method="exact" was too redundant and didn't work well with data and had terrible covariate balance because there are way more values in the control group than treated group

table(clean_relevant_df$Mother.Incarcerated, matching$weights)

fit<-lm(Highschool.Graduated~Mother.Incarcerated+Household.Income+Neighborhood.Safety,
         data=clean_relevant_df,
...
```{r}
#Estimate Causal Effect-coefficient on treatment in the linear regression we fit on the matched data

fit$coefficients["Mother.Incarcerated"]
